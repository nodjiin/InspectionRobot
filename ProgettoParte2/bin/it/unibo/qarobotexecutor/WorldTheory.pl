%==============================================
% WorldTheory.pl for actor qarobotexecutor
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatuqarobotexecutor).	%%old version (deprecated)
actorobj(qatuqarobotexecutor).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor qarobotexecutor
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of qarobotexecutor" ). 
...
loop(I,N,Op) :-  
		...
		%%output( values( I,V,N ) ),
 		V =< N ,!,
   		%% qatuqarobotexecutor <- println( loop( I,V ) ),
...
%-------------------------------------------------
%  User static rules about qarobotexecutor
%------------------------------------------------- 
/*
------------------------------------------------------------------------
...
actorOp( Op )   :- % actorPrintln( actorOp( Op  ) ),
				   java_catch(
						qatuqarobotexecutor <- Op returns R, 	%% R unbound if void
...
%% setActionResult : qatuqarobotexecutor register (in Java) the result under name 'actoropresult'
...
cvtToString( V , "void" ):-
	unbound(V),!,
	qatuqarobotexecutor <- setActionResult( void ).
cvtToString( V , S ):-
	number(V),!,
	setResult(V),
	text_term( S, V ).
cvtToString( V , S ):-
	...
	qatuqarobotexecutor <- setActionResult( V ),
	!.
actorOpResult( V )   :- result(V),!.	
actorOpResult( Res ) :- actoropresult <- toString returns Res, !.
%% actorOpResult( "" ).

androidConsult(T) :- qatuqarobotexecutor <- androidConsult(T), !.
