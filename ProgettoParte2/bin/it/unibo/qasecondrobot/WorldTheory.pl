%==============================================
% WorldTheory.pl for actor qasecondrobot
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatuqasecondrobot).	%%old version (deprecated)
actorobj(qatuqasecondrobot).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor qasecondrobot
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of qasecondrobot" ). 
...
loop(I,N,Op) :-  
		...
		%%output( values( I,V,N ) ),
 		V =< N ,!,
   		%% qatuqasecondrobot <- println( loop( I,V ) ),
...
%-------------------------------------------------
%  User static rules about qasecondrobot
%------------------------------------------------- 
/*
------------------------------------------------------------------------
...
actorOp( Op )   :- % actorPrintln( actorOp( Op  ) ),
				   java_catch(
						qatuqasecondrobot <- Op returns R, 	%% R unbound if void
...
%% setActionResult : qatuqasecondrobot register (in Java) the result under name 'actoropresult'
...
cvtToString( V , "void" ):-
	unbound(V),!,
	qatuqasecondrobot <- setActionResult( void ).
cvtToString( V , S ):-
	number(V),!,
	setResult(V),
	text_term( S, V ).
cvtToString( V , S ):-
	...
	qatuqasecondrobot <- setActionResult( V ),
	!.
actorOpResult( V )   :- result(V),!.	
actorOpResult( Res ) :- actoropresult <- toString returns Res, !.
%% actorOpResult( "" ).

androidConsult(T) :- qatuqasecondrobot <- androidConsult(T), !.
