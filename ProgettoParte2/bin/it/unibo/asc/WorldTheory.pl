%==============================================
% WorldTheory.pl for actor asc
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatuasc).	%%old version (deprecated)
actorobj(qatuasc).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor asc
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of asc" ). 
...
actorOp( Op )   :- % actorPrintln( actorOp( Op  ) ),
				   java_catch(
						qatuasc <- Op returns R, 	%% R unbound if void
...
%% setActionResult : qatuasc register (in Java) the result under name 'actoropresult'
