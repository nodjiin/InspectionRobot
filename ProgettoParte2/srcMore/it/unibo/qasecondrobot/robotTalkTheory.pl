...
robotgui([CMD|CMDTAIL], [DUR|DURTAIL]) :-
/*	actorPrintln("Sono in robot gui prima della ricorsione"),
		 	length(DURTAIL,N_DUR),
	 	 length(CMDTAIL,N_MOV),
	 	 actorPrintln("Lunghezza coda DUR"),
	 	actorPrintln(""+N_DUR),
	 		 	 actorPrintln("Lunghezza coda MOV"),
	 		 	actorPrintln(""+N_MOV),*/
	robotgui(CMDTAIL,DURTAIL),
	actorPrintln(CMD),
	moveCmdTable(CMD, MOVE, SPEED ),
	myname(Robot),
	%% actorPrintln(   usercmd(Robot,MOVE, SPEED) ),
	Angle=0,
	Robot <- execute(MOVE, SPEED, Angle, DUR, '', '').	
...
retrieveMove(N):-
    List <- get(N) returns RETMOVE,
    actorPrintln("Ho recuperato la mossa:"),
	actorPrintln(RETMOVE).


autoReverse() :-
    actorPrintln("Sono in autoreverse"),
	 bagof(MOVE, move(MOVE), MOVES),
	 robotgui(MOVES).
	...
	*/
	...
	/*
	  ...
	  java_class(SourceText, FullClassName, ClassPathList, ObjectRef )
	  	java_object('com.pi4j.io.gpio.impl.GpioControllerImpl',[],Controller),
	  	%%class("com.pi4j.io.gpio.GpioFactory") <- getInstance returns Controller,
	  	Controller <- provisionDigitalOutputPin('RaspiPin.GPIO_06', 'PinState.LOW') returns Led.	  	  	
	  */
  

          blinkLed :-
                class("it.unibo.qalearningrobot.LedUtils") <- startBlinking.
          stopLed :-
          class("it.unibo.qalearningrobot.LedUtils") <- stopBlinking.
